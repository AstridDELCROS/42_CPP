...
Form::Form(const std::string name, const int requiredGradeToSign, const int requiredGradeToExe):_name(name), _isSigned(false), _requiredGradeToSign(requiredGradeToSign), _requiredGradeToExe(requiredGradeToExe){
...
std::ostream&   operator<<(std::ostream &output, Form &rhs) {
    ...
    output << "Minimum grade to execute : " << rhs.getRequiredGradeToExe();
    return output;
}
